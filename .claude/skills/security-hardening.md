...
Start strict, relax as needed. Use `nonce` for inline scripts instead of `unsafe-inline`.
...
Use sliding window algorithm. Store counters in Redis. Return `429` with `Retry-After` header.
...
Run dependency audits in CI on every PR. Block merges on critical/high vulnerabilities.
